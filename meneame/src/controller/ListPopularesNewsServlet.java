...
/**
 * Servlet implementation class ListPopularesNewsServlet
 */
@WebServlet("/ListPopularesNewsServlet")
public class ListPopularesNewsServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public ListPopularesNewsServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<News> newsList = newsDAO.getNewsMostPopular();
		...
		Map<News, User> popNewsMap = new LinkedHashMap<News, User>();
		News news;
		if (newsList != null) {
			for (int i = 0; i < newsList.size(); i++) {
				if (newsList.size() > i) {
					news = newsList.get(i);
					popNewsMap.put(news, userDAO.get(news.getOwner()));
				}
			}
		}

		request.setAttribute("newsMap", popNewsMap);
